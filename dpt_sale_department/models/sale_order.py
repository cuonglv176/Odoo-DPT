...
class SaleOrder(models.Model):
    ...
    def process_domain(self, domain):
        ...
        if self.env.context.get('separate_purchase_for_department', False):
            ...
            domain = AND([domain, [('sale_service_ids.service_id.service_type_id', 'in', service_purchase_type_id.ids)]])
        if self.env.context.get('separate_inventory_for_department', False):
            ...
            domain = AND([domain, [('sale_service_ids.service_id.service_type_id', 'in', service_inventory_type_id.ids)]])
