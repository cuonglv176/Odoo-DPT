...
class DPTServiceCombo(models.Model):
    ...
    currency_id = fields.Many2one('res.currency', string='Tiền tệ', default=lambda self: self.env.company.currency_id, tracking=True)
    ...
    service_line_ids = fields.One2many('dpt.service.combo.line', 'combo_id', string='Dịch vụ trong gói', copy=True)
    ...
    def _compute_total_services(self):
        for combo in self:
            combo.total_services = len(combo.service_line_ids)
...
class DPTServiceComboLine(models.Model):
    _name = 'dpt.service.combo.line'
    _description = 'DPT Service Combo Line'
    _order = 'sequence, id'
    
    sequence = fields.Integer(string='Thứ tự', default=10)
    combo_id = fields.Many2one('dpt.service.combo', string='Gói combo', ondelete='cascade', required=True)
    service_id = fields.Many2one('dpt.service.management', string='Dịch vụ', required=True,
                                domain=[('child_ids', '=', False)], 
                                help="Chỉ chọn dịch vụ không có dịch vụ con")
    description = fields.Text(string='Mô tả')
    quantity = fields.Float(string='Số lượng', default=1.0)
    uom_id = fields.Many2one('uom.uom', string='Đơn vị tính', related='service_id.uom_id', readonly=True)
    price_unit = fields.Float(string='Đơn giá', related='service_id.price', readonly=True)
